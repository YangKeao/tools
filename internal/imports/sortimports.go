...
func sortImports(opts *Options, fset *token.FileSet, f *ast.File) {
		...
		if !opts.CombineGroups {
			...
			for j, s := range d.Specs {
				if j > i && fset.Position(s.Pos()).Line > 1+fset.Position(d.Specs[j-1].End()).Line {
					...
					specs = append(specs, sortSpecs(opts.LocalPrefix, fset, f, d.Specs[i:j])...)
					i = j
				}
			}
			specs = append(specs, sortSpecs(opts.LocalPrefix, fset, f, d.Specs[i:])...)
			d.Specs = specs
		} else {
			d.Specs = sortSpecs(opts.LocalPrefix, fset, f, d.Specs)
		}
